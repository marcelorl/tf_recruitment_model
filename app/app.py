...
def main():
    ...
    with st.form(key='candidate_form'):
        ...
        with col1:
            ...
            gender = st.selectbox("Sexo", options=["Selecione...", "Masculino", "Feminino"])
            education_level = st.selectbox("Nível Educacional", options=[
                "Selecione...", "Ensino Médio", "Graduação", "Mestrado", "Doutorado"])
            experience_years = st.number_input("Experiência (anos)", min_value=0, max_value=50, step=1)
        
        with col2:
            interview_score = st.slider("Pontuação na Entrevista", min_value=0.0, max_value=10.0, step=0.1)
            skill_score = st.slider("Pontuação nas Skills Técnicas", min_value=0.0, max_value=10.0, step=0.1)
            personality_score = st.slider("Pontuação na Personalidade", min_value=0.0, max_value=10.0, step=0.1)
            recruitment_strategy = st.selectbox("Estratégia de Recrutamento", options=[
                "Selecione...", "Estratégia 1", "Estratégia 2", "Estratégia 3"])
            previous_companies = st.number_input("Empresas Anteriores", min_value=0, max_value=20, step=1)
            distance_from_company = st.number_input("Distância da Empresa (km)", min_value=0.0, max_value=1000.0, step=0.1)
